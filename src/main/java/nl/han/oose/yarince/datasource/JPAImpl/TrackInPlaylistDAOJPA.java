 ...
 */
@Alternative
public class TrackInPlaylistDAOJPA implements ITrackInPlaylistDAO {

    @Inject
    private IJPAConnection connection;

    @Inject
    private TrackDAOJPA trackDAOJPA;

    @Override
    public void add(PlaylistEntry playlistEntry, int playlistId) {
        try (
                Connection con = connection.getConnection();
                PreparedStatement preparedStatement = con.prepareStatement("INSERT IGNOREs INTO track_in_playlist (PLAYLIST_ID, TRACK_ID,OFFLINE_AVAILABLE) VALUES (?,?,?)")
        ) {
            ...
            System.out.println("Track in playlist Aded!");
    ...
    private PlaylistEntry getPlaylistEntryByResultSet(ResultSet resultSet) throws SQLException {
        return new PlaylistEntry(trackDAOJPA.findById(resultSet.getInt("TRACK_ID")), resultSet.getBoolean("OFFLINE_AVAILABLE"));
    }
}
